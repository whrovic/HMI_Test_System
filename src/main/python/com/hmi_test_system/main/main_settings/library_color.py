...
class LibraryColor(Lib):
    ...
    def edit_color_edit_color_edit_name(color: Color):
        ...
        new_color_name = Lib.get_input_str("Write the new name of the color: ")
        ...
        for c in ListOfColors.get_list_of_colors():
            if c is color:
                ...
                Lib.exit_input("Invalid color name! This name is already in use")
                return -1
        color.set_name(new_color_name)
        ListOfColors.save_to_file()
        Lib.exit_input("Name changed successfully")
        return 0

    @staticmethod
    def edit_color_edit_color_edit_1st_range(color: Color):
        # Get the hsv values for the 1st range
        hsv_min1 = Lib.get_hsv_values("Write the minimum HSV values of the new color [h s v]: ")
        if len(hsv_min1) != 3: return -1

        hsv_max1 = Lib.get_hsv_values("Write the maximum HSV values of the new color [h s v]: ")
        if len(hsv_max1) != 3: return -1

        # Check for range consistency
        if hsv_min1[0] >= hsv_max1[0]:
            Lib.exit_input("Invalid hsv range! The minimum value is higher than the maximum value")
            return -1
        # Check if the 1st range overlaps the second
            ...
            Lib.exit_input("Invalid hsv range! The ranges cannot overlap inside the color")
            return -1
        ...
        for c in ListOfColors.get_list_of_colors():
            ...
            # Check with 1st range
                ...
                Lib.exit_input("Invalid hsv range! The ranges cannot overlap between different colors")
                return -1
        ...
        ListOfColors.save_to_file()
        Lib.exit_input("1st HSV range changed successfully")
        return 0
    ...
    def edit_color_edit_color_edit_2nd_range(color: Color):
        ...
        if color.get_hsv_min2() is not None:
            remove = Lib.get_yes_no_confirmation("Do you want to remove the 2nd HSV range [y|n]?")
            if remove is None: return -1

            if remove:
                color.set_hsv_min2(None)
                color.set_hsv_max2(None)
                Lib.exit_input("2nd HSV range deleted successfully")
                return 0

        # Get the hsv values for the 1st range
        hsv_min2 = Lib.get_hsv_values("Write the minimum HSV values of the new color [h s v]: ")
        if len(hsv_min2) != 3: return -1

        hsv_max2 = Lib.get_hsv_values("Write the maximum HSV values of the new color [h s v]: ")
    ...
    def edit_color_new_color():
        ...
        if is_there_a_second_range:
            hsv_min2 = Lib.get_hsv_values("Write the minimum HSV values of the new color: ")
            if len(hsv_min2) != 3: return -1

            hsv_max2 = Lib.get_hsv_values("Write the maximum HSV values of the new color: ")
            if len(hsv_max2) != 3: return -1
        ...
        Lib.exit_input(f"Color {new_color_name} added successfully")
        return 0
    ...
    def edit_color_delete_color():
        ...
        remove = Lib.get_yes_no_confirmation(f"Are you really sure you want to delete color {color_name} [y|n]? ")
        if remove is None: return -1
